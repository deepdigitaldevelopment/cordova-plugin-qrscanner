...
gulp.task('webpack-cordova', gulp.series(['prepack'], (cb) =>{
  webpack('webpack.cordova.config.js', cb);
}));

gulp.task('dist', gulp.series(['prepack'], (cb) => {
  webpack('webpack.library.config.js', cb);
}));

gulp.task('remap', gulp.series(['webpack-cordova'], function (cb) {
  return gulp.src(['dist/plugin.min.js', 'dist/www.min.js'])
  .pipe(insert.prepend(remap))
  .pipe(gulp.dest('dist'));
}));

gulp.task('plugin', gulp.series(['remap'], function (cb) {
  return gulp.src(['dist/plugin.min.js'])
  .pipe(gulp.dest('src/browser'));
}));

gulp.task('www', gulp.series(['remap'], function (cb) {
